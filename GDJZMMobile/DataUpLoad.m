...
static NSString *dataNameArray[UPLOADCOUNT]={@"Project",@"Task",@"AtonementNotice",@"CaseDeformation",@"CaseInfo",@"CaseInquire",@"CaseProveInfo",@"CaseServiceFiles",@"CaseServiceReceipt",@"Citizen",@"RoadWayClosed",@"Inspection",@"InspectionCheck",@"InspectionOutCheck",@"InspectionPath",@"InspectionRecord",@"ParkingNode",@"CaseMap",@"ConstructionChangeBack",@"TrafficRecord",@"CaseSpotRecord",@"InspectionLight",@"Pdanotes",@"InspectionHDControl"};
        ...
        if (self.currentWorkIndex < UPLOADCOUNT) {
            ...
            if (self.uploadfailRecord.count!=0) {
                ...
                dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                    ...
                    UIAlertView *finishAlert = [[UIAlertView alloc] initWithTitle:@"消息" message:@"上传失败" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
                    [finishAlert show];
                });
            }else{
                double delayInSeconds = 1.0;
...
- (void)uploadFinished{
    ...
    if (self.currentWorkIndex < UPLOADCOUNT) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            [self.progressView hide];
         UIAlertView *finishAlert = [[UIAlertView alloc] initWithTitle:@"消息" message:@"上传完毕" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
//            UIAlertView *finishAlert=[[UIAlertView alloc]initWithTitle:@"消息" message:@"" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
