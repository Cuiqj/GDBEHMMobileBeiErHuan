...
-(void)setAutoNumberText:(NSString *)aAutoNumber{
    switch (self.pickerType) {
        case kAutoNumber:
        {
            ...
            if (![self.textAutoNumber.text isEqualToString:aAutoNumber]) {
                self.textAutoNumber.text=aAutoNumber;
                [self loadCitizenInfoForCase:self.caseID autoNumber:aAutoNumber andNexus:@"当事人"];
                [self.delegate  setAutoNumber:aAutoNumber];
            }
        }
            break;
        case kNexus:
        {
            ...
            if ([self.textAutoNumber.text isEmpty]) {
                self.textNexus.text=aAutoNumber;
            } else {
                [self loadCitizenInfoForCase:self.caseID autoNumber:self.textAutoNumber.text andNexus:aAutoNumber];
            }            
        }
            break;
        case kCarOwner:
            self.textCarOwner.text=aAutoNumber;
            break;
        default:
            break;
    }
...
-(void)saveCitizenInfoForCase:(NSString *)caseID{
    ...
    if (![aAutoNumber isEmpty] && ![self.textParty.text isEmpty]) {
        Citizen *citizen=[Citizen citizenByAutomobileNumber:aAutoNumber withNexus:self.textNexus.text withCase:caseID];
        if (citizen==nil){
        ...
        //问询笔录是跟当事人相关的，所以需要传递
        CaseInquire* caseInquire = [CaseInquire inquireForCase:self.caseID withAnswererName:citizen.party];
        ...
        if ([self.textNexus.text isEqualToString:@"当事人"]) {
            caseProveInfo = [CaseProveInfo proveInfoForCase:caseID withCitizenName:citizen.party];
        }else if([self.textNexus.text isEqualToString:@"当事人代表"]){
            caseProveInfo = [CaseProveInfo proveInfoForCase:caseID withOrganizer:citizen.party];
        }else if([self.textNexus.text isEqualToString:@"被邀请人"]){
            caseProveInfo = [CaseProveInfo proveInfoForCase:caseID withInvitee:citizen.party];
        }
        ...
        if(caseProveInfo){
            if ([self.textNexus.text isEqualToString:@"当事人"]) {
                ...
                caseProveInfo.organizer = citizen.party;
            }else if([self.textNexus.text isEqualToString:@"被邀请人"]){
                caseProveInfo.invitee = citizen.party;
            }
        }
...
-(void)loadCitizenInfoForCase:(NSString *)caseID autoNumber:(NSString *)aAutoNumber andNexus:(NSString *)nexus{
    ...
    if (![aAutoNumber isEmpty] && aAutoNumber) {
        Citizen *citizen=[Citizen citizenByAutomobileNumber:aAutoNumber withNexus:nexus withCase:caseID];
        if (citizen){
