...
@synthesize citizenInfos =_citizenInfos;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segueIdentifier isEqualToString:@"toCaseDocument"]){
        ...
        documentsVC.parameters = [NSDictionary dictionaryWithObjectsAndKeys:self.inquireInfoBriefVC.textNexus.text,@"relation",self.inquireInfoBriefVC.textParty.text,@"party", nil];
...
- (void)saveCaseInfoForCase:(NSString *)caseID{
    ...
    //因为车辆的信息是保存在当事人里面的，所以如果在主界面修改了当事人的车辆信息，则必须要相应地修改当事人信息
    //所以在一个可以有多车辆的案件里面事先把跟某个车辆相关的人查询出来是很重要的
...
- (void)saveCaseProveInfoForCase:(NSString *)caseID{
    ...
    CaseProveInfo *caseProveInfo = nil;
    if (citizen1) {
        caseProveInfo = [CaseProveInfo proveInfoForCase:caseID withCitizenName:citizen1.party];
    }else if(citizen2){
        caseProveInfo = [CaseProveInfo proveInfoForCase:caseID withOrganizer:citizen2.party];
    }else if(citizen3){
        caseProveInfo = [CaseProveInfo proveInfoForCase:caseID withInvitee:citizen3.party];
    }
    ...
    if(caseProveInfo.event_desc == nil || [caseProveInfo.event_desc isEmpty]){
...
- (void)loadCitizenInfoForCase:(NSString *)caseID andAutoNumber:(NSString *)aAutoNumber{
    ...
    if (fetchResult.count>0) {
        self.citizenInfos= fetchResult;
        Citizen *citizen = [fetchResult objectAtIndex:0];
        self.textBadDesc.text= citizen.bad_desc;
        self.textAutoNumber.text= citizen.automobile_number;
        self.textAutoPattern.text= citizen.automobile_pattern;
        if (self.segInfoPage.selectedSegmentIndex==1) {
...
- (void)saveCitizenInfoForCase:(NSString *)caseID andAutoNumber:(NSString *)aAutoNumber{
    ...
    self.textAutoNumber.text=aAutoNumber.uppercaseString;
    if (![aAutoNumber isEmpty] && aAutoNumber != nil) {
        if (self.citizenInfos == nil || [self.citizenInfos count] < 1) {
            Citizen *citizen=[Citizen newDataObjectWithEntityName:@"Citizen"];
            citizen.party = @"";
            citizen.nexus=@"当事人";
            self.citizenInfos = [[NSMutableArray alloc]init];
            [self.citizenInfos addObject:citizen];
        }
        for (Citizen *citizenInfo in self.citizenInfos){
            citizenInfo.proveinfo_id=caseID;
            citizenInfo.automobile_pattern=self.textAutoPattern.text;
            NSString *autoNumberNew=self.textAutoNumber.text;
            citizenInfo.automobile_number=autoNumberNew;
            if ([autoNumberNew length]>1) {
                ...
                NSString *autoNumberOriginal=citizenInfo.automobile_number;
                ...
                if ((![provinceNew isEqualToString:provinceOriginal]) || (![cityCodeNew isEqualToString:cityCodeOriginal])) {
                    citizenInfo.automobile_address=[citizenInfo.automobile_number autoAddressFromAutoNumber];
                }
            }
            citizenInfo.bad_desc=self.textBadDesc.text;
        }
        [[AppDelegate App] saveContext];
    }
}
