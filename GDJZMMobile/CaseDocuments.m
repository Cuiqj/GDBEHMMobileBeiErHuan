...
#ifdef DEBUG
        return @[@"勘验检查笔录",@"询问笔录",@"现场笔录",@"赔（补）偿通知书",@"现场勘验图",@"赔（补）偿清单",@"责令车辆停驶通知书",@"文书送达回证",@"施工整改通知书"];
#else
        return @[@"勘验检查笔录",@"询问笔录",@"现场笔录",@"赔（补）偿通知书",@"现场勘验图",@"赔（补）偿清单",@"责令车辆停驶通知书",@"文书送达回证"];
#endif
    }
...
+ (NSArray *)allDocumentFullNamesForCaseTypeID:(NSString *)caseTypeID
{
    if ([caseTypeID isEqualToString:CaseTypeIDDefault] ||
        [caseTypeID isEqualToString:CaseTypeIDPei]) {
#ifdef DEBUG
        return @[@"公路赔（补）偿案件勘验检查笔录",@"公路赔（补）偿案件询问笔录",@"公路赔（补）偿案件现场笔录",@"公路赔（补）偿通知书",@"路产索赔现场勘查图",@"责令车辆停驶通知书",@"施工整改通知书（回执）"];
#else
        return @[@"公路赔（补）偿案件勘验检查笔录",@"公路赔（补）偿案件询问笔录",@"公路赔（补）偿案件现场笔录",@"公路赔（补）偿通知书",@"路产索赔现场勘查图",@"责令车辆停驶通知书"];
#endif
    }
    else if ([caseTypeID isEqualToString:CaseTypeIDFa]) {
