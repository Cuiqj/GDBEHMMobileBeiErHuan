...
-(void)viewWillAppear:(BOOL)animated{
    switch (self.pickerType) {
        case kAutoNumber:
        {
            self.data=[Citizen allCitizenByCase:self.caseID];
        }
            break;
        case kNexus:
        ...
        case kCarOwnerName:
            ...
            break;
        case kCarOwnerAllName:
            self.data=[Citizen allCitizenByCase:self.caseID];
            break;
        case kParkingNodeAutoNumber:
            self.data = [ParkingNode parkingNodesByCaseID:self.caseID];
            break;
        default:
            break;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    } else if(self.pickerType== kCarOwnerName){
        cell.textLabel.text=[[_data objectAtIndex:indexPath.row] valueForKey:@"party"];
    }else if(self.pickerType== kCarOwnerAllName){
        cell.textLabel.text=[[_data objectAtIndex:indexPath.row] valueForKey:@"party"];
    }else if(self.pickerType == kParkingNodeAutoNumber){
        cell.textLabel.text=[[_data objectAtIndex:indexPath.row] valueForKey:@"citizen_name"];
    }else{
