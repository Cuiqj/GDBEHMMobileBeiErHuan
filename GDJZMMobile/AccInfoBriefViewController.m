...
//    [ParkingNode deleteAllParkingNodeForCase:caseID];
    if (self.swithIsParking.isOn) {
        if (![self.textParkingCarName.text isEmpty]) {
            ...
            NSMutableArray *parkingNodes = [[NSMutableArray alloc]init];
            for (NSString *citizenName in numberArray){
                if (![citizenName isEmpty]){
                    if([ParkingNode parkingNodesForCase:caseID withCitizenName:citizenName] == nil) {
                        ParkingNode *parkingNode=[ParkingNode newDataObjectWithEntityName:@"ParkingNode"];
                        [parkingNodes addObject:parkingNode];
                        ...
                        if (roadSegment) {
                            NSString *pre = roadSegment.place_prefix1;
                            NSString *last = [caseInfo fullCaseMarkAfterK:NO];
                            parkingNode.happenplace = [pre stringByAppendingFormat:@"----%@",last];
                        }

                        parkingNode.address=self.textParkingLocation.text;
                        [[AppDelegate App] saveContext];
                    }else{
                        ParkingNode *parkingNode=[ParkingNode parkingNodesForCase:self.caseID withCitizenName:citizenName];
                        [parkingNodes addObject:parkingNode];
                        parkingNode.address = self.textParkingLocation.text;
                        parkingNode.date_start = [dateFormatter dateFromString:self.textStartTime.text];
                        parkingNode.date_end = [dateFormatter dateFromString:self.textEndTime.text];
            ...
            [self deleteAllParkingNodes:parkingNodes];
        }else {
            self.textEndTime.text=@"";
            self.textStartTime.text=@"";
            self.textParkingLocation.text=@"";
            [self deleteAllParkingNodes];
        }
    }else{
        [self deleteAllParkingNodes];
    }
}
- (void)deleteAllParkingNodes{
...
- (void)deleteAllParkingNodes:(NSArray*)parkingNodes{
    NSManagedObjectContext *context=[[AppDelegate App] managedObjectContext];
    NSEntityDescription *entity=[NSEntityDescription entityForName:@"ParkingNode" inManagedObjectContext:context];
    NSFetchRequest *fetchRequest=[[NSFetchRequest alloc] init];
    [fetchRequest setEntity:entity];
    ...
    if (fetchResult.count>0) {
        for (NSManagedObject *tableInfo in fetchResult) {
            BOOL flag = NO;
            for (ParkingNode *parkingNode in parkingNodes) {
                if(parkingNode == tableInfo){
                    flag = YES;
                    break;
                }
            }
